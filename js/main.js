	...
	setInterval(function() {
		...
		$('.icon-container').find('.icon').each(function () {
			...
			if (distanceToCursor < escapeDistance && !el.data('ignoreMouse')) {
				...
				// try to avoid cursor
				$(this).css({
					top: offset.top + moveVector.y / speedModifier,
					left: offset.left + moveVector.x / speedModifier
				});
				...
				if (distanceToOriginalPosition > returnDistance && el.data('ignoreMouse')) {
					...
					el.css({
						top: offset.top + moveBackVector.y / speedModifier,
						left: offset.left + moveBackVector.x / speedModifier
					});
